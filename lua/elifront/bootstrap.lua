--- ### elifront Core Bootstrap
--
-- This module simply sets up the global `elifront` module.
-- This is automatically loaded and should not be resourced, everything is accessible through the global `elifront` variable.
--
-- @module elifront.bootstrap
-- @copyright 2022
...
elifront.install = _G["elifront_installation"] or { home = vim.fn.stdpath("config") }
elifront.supported_configs = { elifront.install.home }
--- external elifront configuration folder
elifront.install.config = vim.fn.stdpath("config"):gsub("[^/\\]+$", "elifront")
-- check if they are the same, protects against NVIM_APPNAME use for isolated install
if elifront.install.home ~= elifront.install.config then
  vim.opt.rtp:append(elifront.install.config)
  --- supported elifront user conifg folders
  table.insert(elifront.supported_configs, elifront.install.config)
end
...
local function load_module_file(module)
  ...
  for _, config_path in ipairs(elifront.supported_configs) do
...
function elifront.user_opts(module, default, extend)
...
elifront.updater = {
  options = elifront.user_opts("updater", { remote = "origin", channel = "stable" }),
  snapshot = { module = "lazy_snapshot", path = vim.fn.stdpath("config") .. "/lua/lazy_snapshot.lua" },
  rollback_file = vim.fn.stdpath("cache") .. "/elifront_rollback.lua",
}
local options = elifront.updater.options
if elifront.install.is_stable ~= nil then
  options.channel = elifront.install.is_stable and "stable" or "nightly"
end
...
elifront.user_terminals = {}
--- table of language servers to ignore the setup of, configured through lsp.skip_setup in the user configuration
elifront.lsp = { skip_setup = elifront.user_opts("lsp.skip_setup", {}), progress = {} }
--- the default colorscheme to apply on startup
-- elifront.default_colorscheme = elifront.user_opts("colorscheme", "astrotheme", false)
elifront.default_colorscheme = "catppuccin"
