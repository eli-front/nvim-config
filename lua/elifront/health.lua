...
function M.check()
  health.start("elifront")

  health.info("elifront Version: " .. require("astronvim.utils.updater").version(true))
  ...
  local programs = {
    {
      ...
      extra_check = function(program)
        local git_version = require("elifront.utils.git").git_version()
        if git_version then
