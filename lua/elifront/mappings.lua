-- TODO: replace <leader> to <Leader> everywhere in elifront v4 to match vimdoc
local utils = require("elifront.utils")
local get_icon = utils.get_icon
local is_available = utils.is_available
local ui = require("elifront.utils.ui")

local maps = require("elifront.utils").empty_map_table()
...
maps.n["<leader>pA"] = { "<cmd>AstroUpdate<cr>", desc = "elifront Update" }
maps.n["<leader>pv"] = { "<cmd>AstroVersion<cr>", desc = "elifront Version" }
maps.n["<leader>pl"] = { "<cmd>AstroChangelog<cr>", desc = "elifront Changelog" }

-- Manage Buffers
maps.n["<leader>c"] = {
  function()
    require("elifront.utils.buffer").close()
  end,
  desc = "Close buffer",
}
maps.n["<leader>C"] = {
  function()
    require("elifront.utils.buffer").close(0, true)
  end,
  desc = "Force close buffer",
}
maps.n["]b"] = {
  function()
    require("elifront.utils.buffer").nav(vim.v.count > 0 and vim.v.count or 1)
  end,
  desc = "Next buffer",
}
maps.n["[b"] = {
...
maps.n[">b"] = {
  function()
    require("elifront.utils.buffer").move(vim.v.count > 0 and vim.v.count or 1)
  end,
...
maps.n["<leader>bc"] = {
  function()
    require("elifront.utils.buffer").close_all(true)
  end,
  desc = "Close all buffers except current",
}
maps.n["<leader>bC"] = {
  function()
    require("elifront.utils.buffer").close_all()
  end,
  desc = "Close all buffers",
}
maps.n["<leader>bl"] = {
  function()
    require("elifront.utils.buffer").close_left()
  end,
  desc = "Close all buffers to the left",
}
maps.n["<leader>bp"] = {
  function()
    require("elifront.utils.buffer").prev()
  end,
  desc = "Previous buffer",
}
maps.n["<leader>br"] = {
  function()
    require("elifront.utils.buffer").close_right()
  end,
...
maps.n["<leader>bse"] = {
  function()
    require("elifront.utils.buffer").sort("extension")
  end,
  desc = "By extension",
}
maps.n["<leader>bsr"] = {
  function()
    require("elifront.utils.buffer").sort("unique_path")
  end,
  desc = "By relative path",
}
maps.n["<leader>bsp"] = {
  function()
    require("elifront.utils.buffer").sort("full_path")
  end,
  desc = "By full path",
}
maps.n["<leader>bsi"] = {
  function()
    require("elifront.utils.buffer").sort("bufnr")
  end,
  desc = "By buffer number",
}
maps.n["<leader>bsm"] = {
  function()
    require("elifront.utils.buffer").sort("modified")
  end,
...
if is_available("heirline.nvim") then
  maps.n["<leader>bb"] = {
    function()
      require("elifront.utils.status.heirline").buffer_picker(function(bufnr)
  ...
  maps.n["<leader>bd"] = {
    function()
      require("elifront.utils.status.heirline").buffer_picker(function(bufnr)
        require("elifront.utils.buffer").close(bufnr)
      end)
    end,
...
if is_available("telescope.nvim") then
  ...
  maps.n["<leader>fa"] = {
    function()
      ...
      local search_dirs = {}
      for _, dir in ipairs(elifront.supported_configs) do -- search all supported config locations
        if dir == elifront.install.home then
          dir = dir .. "/lua/user"
        end -- don't search the elifront core files
    ...
    desc = "Find elifront config files",
  }
  maps.n["<leader>fb"] = {
    function()
...
if is_available("toggleterm.nvim") then
  ...
  if vim.fn.executable("lazygit") == 1 then
    ...
    maps.n["<leader>gg"] = {
      function()
        local worktree = require("elifront.utils.git").file_worktree()
...
utils.set_mappings(elifront.user_opts("mappings", maps))
