...
if not (vim.uv or vim.loop).fs_stat(lazypath) then -- TODO: REMOVE vim.loop WHEN DROPPING SUPPORT FOR Neovim v0.9
  ...
  vim.api.nvim_create_autocmd("User", {
    ...
    callback = function()
      ...
      require("elifront.utils").notify("Mason is installing packages if configured, check status with `:Mason`")
    end,
  })
end
vim.opt.rtp:prepend(lazypath)

local user_plugins = elifront.user_opts("plugins")
for _, config_dir in ipairs(elifront.supported_configs) do
  if vim.fn.isdirectory(config_dir .. "/lua/user/plugins") == 1 then
    user_plugins = { import = "user.plugins" }
  end
end

local spec = elifront.updater.options.pin_plugins and { { import = elifront.updater.snapshot.module } } or {}
vim.list_extend(spec, { { import = "plugins" }, user_plugins })

local colorscheme = elifront.default_colorscheme and { elifront.default_colorscheme } or nil

require("lazy").setup(elifront.user_opts("lazy", {
  spec = spec,
  defaults = { lazy = true },
  ...
  performance = {
    rtp = {
      paths = elifront.supported_configs,
