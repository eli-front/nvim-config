...
local git = require("elifront.utils.git")
...
function M.generate_snapshot(write)
  ...
  table.sort(plugins, function(l, r)
    return l[1] < r[1]
  end)
...
function M.version(quiet)
  local version = elifront.install.version or git.current_version(false) or "unknown"
  if elifront.updater.options.channel ~= "stable" then
...
function M.create_rollback(write)
  ...
  if write == true then
    local file = assert(io.open(elifront.updater.rollback_file, "w"))
...
function M.rollback()
  local rollback_avail, rollback_opts = pcall(dofile, elifront.updater.rollback_file)
...
function M.update_available(opts)
  if not opts then
    opts = elifront.updater.options
  end
  opts = require("elifront.utils").extend_tbl({ remote = "origin" }, opts)
...
function M.update(opts)
  ...
  opts = require("elifront.utils").extend_tbl(
