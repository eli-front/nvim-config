...
autocmd({ "BufAdd", "BufEnter", "TabNewEntered" }, {
  ...
  callback = function(args)
    local buf_utils = require("elifront.utils.buffer")
...
autocmd("BufDelete", {
  ...
  callback = function(args)
    ...
    vim.t.bufs = vim.tbl_filter(require("elifront.utils.buffer").is_valid, vim.t.bufs)
    if removed then
      astroevent("BufsUpdated")
    end
...
autocmd("BufEnter", {
  desc = "Quit elifront if more than one window is open and only sidebar windows are list",
...
autocmd("ColorScheme", {
  ...
  group = augroup("elifront_highlights", { clear = true }),
  callback = function()
    if vim.g.colors_name then
      for _, module in ipairs({ "init", vim.g.colors_name }) do
        for group, spec in pairs(elifront.user_opts("highlights." .. module)) do
...
autocmd({ "BufReadPost", "BufNewFile", "BufWritePost" }, {
  desc = "elifront user events for file detection (AstroFile and AstroGitFile)",
  ...
  callback = function(args)
    ...
    if not (current_file == "" or vim.api.nvim_get_option_value("buftype", { buf = args.buf }) == "nofile") then
      astroevent("File")
      if
          require("elifront.utils.git").file_worktree()
...
cmd("AstroChangelog", function()
  require("elifront.utils.updater").changelog()
end, { desc = "Check elifront Changelog" })
cmd("AstroUpdatePackages", function()
  require("elifront.utils.updater").update_packages()
end, { desc = "Update Plugins and Mason" })
cmd("AstroRollback", function()
  require("elifront.utils.updater").rollback()
end, { desc = "Rollback elifront" })
cmd("AstroUpdate", function()
  require("elifront.utils.updater").update()
end, { desc = "Update elifront" })
cmd("AstroVersion", function()
  require("elifront.utils.updater").version()
end, { desc = "Check elifront Version" })
cmd("AstroReload", function()
  require("elifront.utils").reload()
end, { desc = "Reload elifront (Experimental)" })
